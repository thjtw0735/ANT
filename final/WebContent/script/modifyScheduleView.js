...
function test321(Tdate, Ttime, Tcontent, Ttitle, Taddress, Scontent, Ssubject) {
	for (var i = 0; i < 19; i++) {
	...
	for(var i=0; i<Tdate.length; i++) {	
		if(Taddress[i]!="null") {
			table[Tdate[i]][Ttime[i]] = Taddress[i];
		}
		if(Ttitle[i]!="null") {
			tablePlace[Tdate[i]][Ttime[i]] = Ttitle[i];
			addStory(Ttitle[i],s++,Taddress[i]);
		}
		
		if( Tcontent[i]=="null") {
			// tableContent[Tdate[i]][Ttime[i]] = Tcontent[i];
		} else {
			tableContent[Tdate[i]][Ttime[i]] = Tcontent[i];
		}
	...
	for(var i=0; i<Scontent.length; i++) {
		var storyindex = document.getElementById(i);
		var tmp = document.createElement('div');
		
		var tmp2 = '<form id="postForm"><div id = "post'+i+'" class="postItem">'
		+'<h2 name="postSubject" class="postSubject">'+Ssubject[i]+'</h2>'
		+'<h5 name="postContent" class="postContent">'
		+Scontent[i]+'</h5></div></form>';
		tmp.innerHTML=tmp2;
...
*/ 
	...
	if(splittmp[0]=="td") {
		...
		btnElement.innerHTML += "<button onclick=deletePosition('"+ target.id.split("(")[1] + "') class='btn1'>삭제</button>";
...
function Grid(name, data)
{
	...
	for ( var i = 0; i < this.data.nRow; i++ )
	{
		...
		aStr.push('<th valign=top width="60"><span>' + this.data.rowHeaders[i] + '</span></th> <td width="60">aa </td>');
	...
	for ( i = 0; i < this.data.nRow; i++ ) {
		for ( j = 0; j < this.data.nCol; j++ ) {
			...
			if(tablePlace[i][j + xOffset]) {
				...
				this.aCellSpan[i][j].parentNode.innerHTML += '<h5 id="123_'+ numbertmp++ + '" draggable="true" ondragstart="drag( this, event,'+"\'"+tablePlace[i][j + xOffset]+"','"+ table[i][j + xOffset]+"\'" +')">' + tablePlace[i][j + xOffset] 
				+ "<button onclick=deletePosition('"+ i+","+j + "') class='btn1'>삭제</button></h5>";
			}
			...
			if(tableContent[i][j + xOffset]) {
				...
				this.aCellSpan[i][j].innerHTML = " &nbsp;";	
				this.aCellSpan[i][j].parentNode.innerHTML += "<input id=text("+i+","+j+") value="+  tableContent[i][j + xOffset] +" type='text' id=text("+i+","+(j+xOffset)+")  onKeyup=checkContent('"+i+"','"+j+"')>";
...
function deletePosition(address) {
	// markPoint("title", address);
...
function ModifyScheduleDB(schedule_num) {
	...
	window.location = "modifySchedulePro.do?" + params + "&schedule_num=" + schedule_num;
	var url = "modifySchedulePro.do?" + params + "&schedule_num="+schedule_num
...
function addPost(index){
	...
	if(ischeck==null){
		...
		open('createPost.do?index='+index+'&content='+$('#post'+index+'>.postContent').html() + '&subject='+$('#post'+index+">.postSubject").html()+'&imgCnt='+imgCnt,'','width=750, height=600,left=400, top=0, toolbar=no, scrollbars=yes, resizeable=no');
	}
